...
class ESIM(object):
    def __init__(self, seq_length, n_vocab, embedding_size, hidden_size, n_classes, batch_size, learning_rate, optimizer, l2, clip_value):
        # model init
        self._parameter_init(seq_length, n_vocab, embedding_size, hidden_size, n_classes, batch_size, learning_rate, optimizer, l2, clip_value)
        self._placeholder_init()

        # model operation
        self.logits = self._logits_op()
        self.loss = self._loss_op()
        self.acc,self.P = self._acc_op()
        self.train = self._training_op()

        tf.add_to_collection('train_mini', self.train)

    # init hyper-parameters
    def _parameter_init(self, seq_length, n_vocab, embedding_size, hidden_size, n_classes, batch_size, learning_rate, optimizer, l2, clip_value):
        """
    ...
    def _placeholder_init(self):
        ...
        self.y_input=tf.placeholder(tf.int32, [None,], 'y_input')
        self.y = tf.one_hot(self.y_input,2,name='y_true')
    ...
    def _localInferenceBlock(self, a_bar, b_bar, scope):
        ...
        """
        with tf.variable_scope(scope):
            # e = a_bar.T * b_bar (11) 向量内积，相似为1，不相似为0  这里是biLSTM过后的向量相似度
            ...
            attentionSoft_b = tf.nn.softmax(tf.transpose(attentionWeights,[0,2,1]))
    ...
    def _loss_op(self, l2_lambda=0.0001):
        with tf.name_scope('cost'):
            ...
            self.soft=tf.nn.softmax(logits=self.logits)
            coe = tf.constant([1.0, 4.0])
    ...
    def _acc_op(self):
        with tf.name_scope('acc'):
            ...
            label_pred=tf.cast(label_pred,tf.int32)
            label_true=tf.cast(label_true,tf.int32)

            TP = tf.count_nonzero(label_true*label_pred)
            FP = tf.count_nonzero(label_pred * (label_true - 1))
            FN = tf.count_nonzero((label_pred - 1) * label_true)
